...
% Site information
fname = 'US-WPT_WT.nc';   % 2011 - 2013
casename = 'Twitchell island cropland';
edgen = 38.7998;
edges = 38.2998;
edgee = -121.5151;
edgew = -122.0151;
...
%% Read in data values
rawdata = csvread('US-WPT.csv', 2, 0);
tstep_per_day = 48;
day = 365;
yr = 3;
...
%% Specifically for US-WPT to fix the unreasonable spline interpolation
%% at the beginning of the first year
wt_filled(wt_filled>1) = nan;
wt_filled(wt_filled<0) = nan;
%% Multi-year mean method to fill the gap in water table depth
multi_yr_mean_wt = zeros(48*365,1);
for i=1:(48*365)
