...
inputfile = {'Coastal_Sites\AMF_US-EDN_BASE-BADM_1-5\AMF_US-EDN_BASE_HH_1-5.csv'};
sitename = 'Eden Landing Ecological Reserve';
lat = [37.6156];
lon = [-122.114];
tstep_sec = '1800';
site_timezone = '-8:00';
metfile = 'US-EDNforcing.nc';
fname = 'US-EDN-CRUNCEP.nc';
height = 30;
nyr = 1;
year_tot = [2018];

%% For this site, October, November and December are not available
%% SWdown and LWdown are not available
...
for i=1:length(inputfile)
    ...
    tair_tot = fulldata(:, 20);
    vpd_tot = fulldata(:, 21);
    psurf_tot = fulldata(:, 18);
    rainf_tot = fulldata(:, 34);
    wind_tot = fulldata(:, 11);
    rh_tot = fulldata(:, 19);
    aco2_tot = fulldata(:, 3);
end
...
% 3. Not able to get multiple year means to perform copyfill since this
% site only has one year data
