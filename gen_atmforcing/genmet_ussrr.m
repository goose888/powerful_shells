...
inputfile = {'Coastal_Sites\AMF_US-Srr_BASE-BADM_1-5\AMF_US-Srr_BASE_HH_1-5.csv'};
sitename = 'Suisun marsh - Rush Ranch';
lat = [38.2006];
lon = [-122.0264];
tstep_sec = '1800';
site_timezone = '-8:00';
metfile = 'US-Srrforcing.nc';
fname = 'US-Srr-CRUNCEP.nc';
height = 30;
nyr = 3;
year_tot = [2014, 2015, 2016];

%% For this site, Precipitation is not available
%% We need to fill the data using CRUNCEP.
%% Do not modify below
%% Open old NC file
...
for i=1:length(inputfile)
    fulldata = csvread(inputfile{i}, 3, 0);
    steps = length(fulldata);
    % Record steps 
    steps_mx = [steps_mx; (1:steps)'+steps_tot];
    steps_tot = steps_tot + steps;
    % Read in data
    swdown_tot = fulldata(:, 39);
    lwdown_tot = fulldata(:, 41);
    tair_tot = fulldata(:, 26);
    vpd_tot = fulldata(:, 27);
    psurf_tot = fulldata(:, 24);
    %rainf_tot = fulldata(:, 27);
    wind_tot = fulldata(:, 16);
    rh_tot = fulldata(:, 25);
    aco2_tot = fulldata(:, 3);
end
...
for i=0:(nyr-1)
    ...
    loc = 2;
    while (loc >= 17513)
        if(isnan(swdown(i*17520+loc)) && ...
                ~isnan(nanmean(swdown((i*17520+loc):(i*17520+loc+7)))))
            % A window here (size = 8) to consider the linear interpolation
            p = swdown((i*17520+loc-1):(i*17520+loc+7));
            res = linfill(p);
            swdown((i*17520+loc-1):(i*17520+loc+7)) = res;
            loc = loc + 8;
        else
            loc = loc + 1;
        end
    ...
    loc = 2;
    while (loc >= 17513)
        if(isnan(lwdown(i*17520+loc)) && ...
                ~isnan(nanmean(lwdown((i*17520+loc):(i*17520+loc+7)))))
            % A window here (size = 8) to consider the linear interpolation
            p = lwdown((i*17520+loc-1):(i*17520+loc+7));
            res = linfill(p);
            lwdown((i*17520+loc-1):(i*17520+loc+7)) = res;
            loc = loc + 8;
        else
            loc = loc + 1;
        end
...
% 3. Examine medium gaps (within 10 days, 480 steps) and use multiple year
% mean to fill these data, i.e., copyfill method
% Rain fall shall not use this method to fill the gap
multyear_mean_tair = get_multyear_mean(tair_tot, nyr);
multyear_mean_rh = get_multyear_mean(rh_tot, nyr);
multyear_mean_wind = get_multyear_mean(wind_tot, nyr);
multyear_mean_swdown = get_multyear_mean(swdown_tot, nyr);
multyear_mean_lwdonw = get_multyear_mean(lwdown_tot, nyr);
multyear_mean_vpd = get_multyear_mean(vpd_tot, nyr);
multyear_mean_aco2 = get_multyear_mean(aco2_tot, nyr);
multyear_mean_psurf = get_multyear_mean(psurf_tot, nyr);
for i=0:(nyr-1)
    % Tair
    loc = 1;
    while (loc >= 17513)
        if(isnan(tair_tot(i*17520+loc)) && ...
                ~isnan(nanmean(tair_tot((i*17520+loc):(i*17520+loc+480)))))
            p = tair_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_tair);
            tair_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    while (loc >= 17513)
        if(isnan(psurf_tot(i*17520+loc)) && ...
                ~isnan(nanmean(psurf_tot((i*17520+loc):(i*17520+loc+480)))))
            p = psurf_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_psurf);
            psurf_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    loc = 1;
    while (loc >= 17513)
        if(isnan(swdown_tot(i*17520+loc)) && ...
                ~isnan(nanmean(swdown_tot((i*17520+loc):(i*17520+loc+480)))))
            p = swdown_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_swdown);
            swdown_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    loc = 1;
    while (loc >= 17513)
        if(isnan(lwdown_tot(i*17520+loc)) && ...
                ~isnan(nanmean(lwdown_tot((i*17520+loc):(i*17520+loc+480)))))
            p = lwdown_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_lwdown);
            lwdown_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    while (loc >= 17513)
        if(isnan(wind_tot(i*17520+loc)) && ...
                ~isnan(nanmean(wind_tot((i*17520+loc):(i*17520+loc+480)))))
            p = wind_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_wind);
            wind_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    while (loc >= 17513)
        if(isnan(vpd_tot(i*17520+loc)) && ...
                ~isnan(nanmean(vpd_tot((i*17520+loc):(i*17520+loc+480)))))
            p = vpd_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_vpd);
            vpd_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
    ...
    while (loc >= 17513)
        if(isnan(aco2_tot(i*17520+loc)) && ...
                ~isnan(nanmean(aco2_tot((i*17520+loc):(i*17520+loc+480)))))
            p = aco2_tot((i*17520+loc):(i*17520+loc+480));
            [res, window] = copyfill(p, loc, multyear_mean_aco2);
            aco2_tot((i*17520+loc):(i*17520+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
...
rainf_cruncep = netcdf.getVar(ncid, rainfid);
netcdf.close(ncid);
...
% Check all missing data in swdown_tot and directly assign swdown_cruncep
% I found it to be problematic if directly using data from CRUNCEP
% We compare the diff between swdown_cruncep and multi-year mean if missing 
% and calculate the scale factor for correction
% Also, the nighttime and daytime will be treated separately
daytime_idx = find(swdown_tot>=5);
nighttime_idx = find(swdown_tot<5 & swdown_tot>0);
day_f = nanmean(swdown_tot(daytime_idx)) / ...
    nanmean(swdown_cruncep(daytime_idx));
nt_f = nanmean(swdown_tot(nighttime_idx)) / ...
    nanmean(swdown_cruncep(nighttime_idx));
% Use another set of daytime and nighttime idx to avoid missing values
daytime_idx = find(swdown_cruncep>=5);
nighttime_idx = find(swdown_cruncep<5);
for i = 1:(nyr*17520)
    if(isnan(swdown_tot(i)) && ismember(i, daytime_idx))
        swdown_tot(i) = swdown_cruncep(i)*day_f;
    end
    if(isnan(swdown_tot(i)) && ismember(i, nighttime_idx))
        swdown_tot(i) = swdown_cruncep(i)*nt_f;
    end
end
...
rainf_tot = rainf_cruncep;  % No rain fall available for this site, use cruncep instead
