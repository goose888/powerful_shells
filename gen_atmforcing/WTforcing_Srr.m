...
% Site information
fname = 'US-Srr_WT.nc';   % 2011 - 2013
casename = 'Suisun marsh - Rush Ranch';
edgen = 38.5;
edges = 38.0;
edgee = -121.75;
edgew = -122.25;
...
%% Read in data values
rawdata = csvread('Coastal_Sites\AMF_US-Srr_BASE-BADM_1-5\AMF_US-Srr_BASE_HH_1-5.csv', 3, 0);
tstep_per_day = 48;
day = 365;
nyr = 3;
...
%% This site is a relatively shallow tidal marsh
%% So we here apply the mean instead of submerged
%% water table depth
...
% 2. Multi-year mean method to fill the gap in water table depth
% Multi-year mean is NOT a good method, but we have to use since no data
% available
multi_yr_mean_wt = zeros(48*365,1);
for i=1:17520
    temp = [];
    for j=0:(nyr-1)
        temp = [temp, wt(i+j*17520)];
    end
    multi_yr_mean_wt(i) = nanmean(temp);
end

% Use multi-year mean value to fill the missing value first
for j = 0:(nyr-1)
    for i=1:length(multi_yr_mean_wt)
        if(isnan(wt(j*17520+i)))
            wt(j*17520+i) = multi_yr_mean_wt(i); 
        end
    end
end

% % Currently not applied
% % 3. Fill the gap by spline interpolation
...
% If not able to fill the gap, force it at the mean water table depth level
meanwt = nanmean(wt);
for i=1:(nyr*17520)
    if (isnan(wt(i)))
        wt(i) = meanwt;
