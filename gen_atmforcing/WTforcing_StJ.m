...
% Site information
fname = 'US-StJ_WT.nc';   % 2011 - 2013
casename = 'Sanit Jones Reserve';
edgen = 39.7879;
edges = 39.2879;
edgee = -75.9038;
edgew = -76.4038;
...
%% Read in data values
rawdata = csvread('Coastal_Sites\AMF_US-StJ_BASE-BADM_1-5\AMF_US-StJ_BASE_HH_1-5.csv', 4, 0);
tstep_per_day = 48;
day = 365;
nyr = 1;
...
% 2. Cannot use multi-year mean value to fill the missing value
% Since we only have one year data.
% SWC for this site does not show a good correlation to the water table
% depth.
% Hence we gave up using correlation between SWC and WTD.

% 3. If still not able to fill the gap, force it as surface water
for i=1:(nyr*17520)
    if (isnan(wt(i)))
        wt(i) = -0.001;
    end
