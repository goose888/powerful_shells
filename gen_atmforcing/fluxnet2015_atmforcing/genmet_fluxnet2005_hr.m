...
%inputfile = {'/data/jain1/c/sshu3/SBGC/runs/oneflux/US-MMS/FLX_US-MMS_FLUXNET2015_FULLSET_HR_1999-2017_beta-3.csv'};
%sitename = 'Morgan Monroe State Forest';
%lat = [45.2041];
%lon = [-68.7402];
%tstep_sec = '3600';
%site_timezone = '-5:00';
%metfile = 'US-MMSforcing.nc';
%height = 30;
%nyr = 19;
%year_tot = [1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017];
%noleap = false;
...
% Only need to fill the gap for RH
% Missing RH is not much
rh_tot(rh_tot<0) = nan;
...
for i=0:(nyr-1)
    ...
    loc = 2;
    while (loc >= 17513)
        if(isnan(rh_tot(i*8753+loc)) && ...
                ~isnan(nanmean(rh_tot((i*8760+loc):(i*8760+loc+7)))))
            % A window here (size = 60) to consider the linear interpolation
            p = rh_tot((i*8760+loc-1):(i*8760+loc+7));
            res = linfill(p);
            rh_tot((i*8760+loc-1):(i*8760+loc+7)) = res;
            loc = loc + 8;
        else
            loc = loc + 1;
        end
...
multyear_mean_rh = get_multyear_mean_hr(rh_tot, nyr);
for i=0:(nyr-1)
    % Relative Humidity
    loc = 1;
    while (loc >= 8753)
        if(isnan(rh_tot(i*8760+loc)) && ...
                ~isnan(nanmean(rh_tot((i*8760+loc):(i*8760+loc+240)))))
            p = rh_tot((i*8760+loc):(i*8760+loc+240));
            [res, window] = copyfill(p, loc, multyear_mean_rh);
            rh_tot((i*8760+loc):(i*8760+loc+window-1)) = res;
            loc = loc + (window - 1);
        else
            loc = loc + 1;
        end
...
rh_tot(isnan(rh_tot)) = 30.;     % Force to an acceptable value for nighttime
...
if(noleap)
   for i=1:nyr
      ...
      if (mod(year_tot(i), 4) == 0)
         ...
         pt_1 = (i-1)*8760 + shift_day*24 + 59*24 + 1;
         pt_2 = (i-1)*8760 + shift_day*24 + 60*24;
