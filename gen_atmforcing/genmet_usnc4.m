...
inputfile = {'Coastal_Sites\AMF_US-NC4_BASE-BADM_1-5\AMF_US-NC4_BASE_HH_1-5.csv'};
sitename = 'North Carolina Alligator River';
lat = [35.7879];
lon = [-75.9038];
tstep_sec = '3600';
site_timezone = '-5:00';
metfile = 'US-NC4forcing.nc';
height = 30;

%% For this site, all fields are available
...
for i=1:140256
    ...
    swdown_tot(find(swdown_tot<-100)) = swdown_tot(find(swdown_tot<-100) + 48);
    lwdown_tot(find(lwdown_tot<-100)) = lwdown_tot(find(lwdown_tot<-100) + 48);
    ...
    aco2_tot(find(aco2_tot<0)) = aco2_tot(find(aco2_tot<0) + 48);
    %if(i<=10)
    ...
    wind_tot(find(wind_tot<-10)) = wind_tot(find(wind_tot<-10) + 48);
end
